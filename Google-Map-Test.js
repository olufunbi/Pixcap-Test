...
describe('Access Test Instance (Google maps page)', () => {
  ...
  describe('Verify that the Rivers, parks and beaches are drawn correctly', () => {
    ...
    //Use Inbuilt cypress snapshot to validate canvas
    cy.wait(2000).screenshot()
      
    // Use appplitools eyes to validate canvas
    cy.eyesOpen({
        appName: 'Google Map Test',
        testName: 'Canvas Test'
    })

    cy.xpath(`//canvas`).wrap().eyesCheckWindow()
    

    // cy.eyesCheckWindow();
    cy.eyesClose();
    
    //Use Percy validation to verify canvas
    cy.percySnapshot('Home layout')
    cy.xpath(`//canvas`).then(Random => {
        cy.wrap(Random).percySnapshot()
    })
...
//For the purpose of Visual Regression Testing, This test has to be run at least twice to establish a baseline Image for the subsequent results to be compared with.
